...
//
// RESPONSÁVEL: quem pediu a lista no painel (login individual) fica como
// responsável (e createdBy) de empresa, contato e negócio. O id do Moskit vem
// no pedido (listaInfo.responsibleId). Só cai no MOSKIT_RESPONSIBLE_ID do .env
// quando o pedido não tem usuário (ex.: geração local sem login).
//
...
function cfg() {
  ...
  return {
    ...
    // Responsável PADRÃO — só quando o pedido não veio com usuário logado.
    defaultUserId: parseInt(process.env.MOSKIT_RESPONSIBLE_ID, 10) || 155074,
  };
}
...
/** Usuários da conta Moskit: [{ id, name, active }]. Guardado por 10 min
 *  (a chamada leva ~2s e a lista quase não muda — sem isso a tela fica lenta). */
let cacheUsers = { em: 0, dados: null };
async function listUsers(c, { maxIdadeMs = 10 * 60 * 1000 } = {}) {
  if (cacheUsers.dados && Date.now() - cacheUsers.em < maxIdadeMs) return cacheUsers.dados;
  const users = await mk(c, 'GET', '/users?quantity=100');
  const lista = (Array.isArray(users) ? users : [])
    .map(u => ({ id: u.id, name: u.name, active: u.active !== false }));
  cacheUsers = { em: Date.now(), dados: lista };
  return lista;
}

/**
 * Decide quem fica como responsável pelos leads DESTA lista: quem pediu
 * (listaInfo.responsibleId), conferido na conta Moskit. Se o id não existe ou
 * está inativo, cai no padrão do .env — e avisa no log, pra ninguém ficar sem lead.
 */
async function resolveResponsible(c, listaInfo, log) {
  const pedido = parseInt(listaInfo.responsibleId, 10) || 0;
  const rotulo = listaInfo.responsavel ? ` (${listaInfo.responsavel})` : '';
  let users = [];
  try { users = await listUsers(c); }
  catch (e) { log(`Moskit: não consegui conferir os usuários da conta (${String(e.message).slice(0, 80)}).`); }
  const nomeDe = id => (users.find(u => u.id === id) || {}).name;

  if (pedido) {
    const u = users.find(x => x.id === pedido);
    if (u && u.active) return { id: pedido, nome: u.name };
    // sem lista pra conferir (API fora?): confia no id que veio do pedido
    if (!users.length) return { id: pedido, nome: listaInfo.responsavel || `#${pedido}` };
    log(`Moskit ⚠️ usuário #${pedido}${rotulo} ${u ? 'está INATIVO' : 'não existe'} na conta — ` +
        'usando o responsável padrão do .env. Confira o moskit_user_id na tabela usuarios.');
  }
  return { id: c.defaultUserId, nome: nomeDe(c.defaultUserId) || `#${c.defaultUserId}`, padrao: true };
}
...
async function uploadLead(c, lead, listaInfo, userId) {
  ...
  const quem = { createdBy: { id: userId }, responsible: { id: userId } };
 ...
 * listaInfo: { cidade, uf, responsibleId?, responsavel? }
 * Retorna { criados, jaExistiam, erros, responsavel, responsavelId }.
 */
async function uploadList(leads, listaInfo, log = () => {}) {
  const c = cfg();
  if (!c) { log('Moskit: sem MOSKIT_API_KEY no .env — upload pulado.'); return null; }

  const resp = await resolveResponsible(c, listaInfo, log);
  log(`Moskit: subindo ${leads.length} leads (funil Outbound → Prospecto, ` +
      `responsável ${resp.nome}${resp.padrao ? ' — padrão do .env' : ''})…`);
  let criados = 0, jaExistiam = 0, erros = 0;
  for (const lead of leads) {
    const org = orgName(lead.displayName || lead.name);
    try {
      const r = await uploadLead(c, lead, listaInfo, resp.id);
  ...
  log(`Moskit: fim — ${criados} criados, ${jaExistiam} já existiam, ${erros} erros (responsável: ${resp.nome}).`);
  return { criados, jaExistiam, erros, responsavel: resp.nome, responsavelId: resp.id };
}

module.exports = { uploadList, listUsers, cfg };
