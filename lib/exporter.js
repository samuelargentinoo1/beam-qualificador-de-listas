...
function exportList({ baseDir, n, segment, city, uf, raw, cleaned, cut, qualified, discarded }) {
  ...
  // No modo planilha cada lead tem a SUA cidade (vinda da Receita) e a planilha
  // pode misturar cidades — então o lead manda, e o rótulo do job é só fallback.
  const cidadeDe = l => (l.city ? titleCase(l.city) : cityT);
  const ufDe = l => (l.uf || uf || '');
  ...
  writeCsv(files.limpa, [
    ...
    ...cleaned.map(c => [c.displayName, cidadeDe(c), ufDe(c), c.phone, c.website]),
  ]);
  ...
  writeCsv(files.final, [
    ...
    ...qualified.map(q => [q.displayName, cidadeDe(q), ufDe(q), q.phone, q.site,
  ...
  for (const q of qualified) {
    ...
    pipeRows.push([
      q.decisor, fn, ln, q.phone, '', '', org, `${cidadeDe(q)}${ufDe(q) ? ', ' + ufDe(q) : ''}`,
