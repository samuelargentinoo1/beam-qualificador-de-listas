...
/**
 * Procura um CNPJ já entregue em QUALQUER praça.
 *
 * O modo planilha precisa disso: lá o CNPJ é conhecido ANTES de qualquer
 * consulta, mas a cidade (e portanto a praça) só aparece depois de bater na
 * Receita. Checando por CNPJ em todas as praças, um lead que já saiu pelo Maps
 * não sai de novo pela planilha — e vice-versa, já que markDelivered indexa
 * por "cnpj:<dígitos>" sempre que o CNPJ é conhecido.
 *
 * Retorna { key, info } da praça onde foi entregue, ou null.
 */
function findDeliveredByCnpj(ledger, cnpjDigits) {
  const digits = String(cnpjDigits || '').replace(/\D/g, '');
  if (digits.length !== 14) return null;
  const id = 'cnpj:' + digits;
  for (const [key, e] of Object.entries(ledger.entries || {})) {
    if (e.delivered && e.delivered[id]) return { key, info: e.delivered[id] };
  }
  return null;
}

module.exports = {
  load, save, keyFor, filterNew, markDelivered, markDiscarded, deliveredCount,
  findDeliveredByCnpj, LEDGER_PATH,
};
