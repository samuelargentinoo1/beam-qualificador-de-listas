 ...
 * BrasilAPI → minhareceita. Retorna null se ambas falharem.
 */
async function fetchCnpjApi(digits) {
  let d = await fetchJson(`https://brasilapi.com.br/api/cnpj/v1/${digits}`);
  if (d && d.razao_social) {
    return {
      fonte: 'BrasilAPI',
  ...
  d = await fetchJson(`https://minhareceita.org/${digits}`);
  if (d && d.razao_social) {
    return {
      fonte: 'minhareceita',
      razao: d.razao_social || '',
...
async function findCnpjAndQsa(page, cand, city, uf, siteInfo, log) {
  ...
  const tryDigits = async (digits, origem) => {
    ...
    if (!data) { apiDown++; log(`API de CNPJ fora do ar p/ ${digits} — vou adiar em vez de descartar`); return null; }
